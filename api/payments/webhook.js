...
export default async function handler(request, response) {
  ...
  try {
    ...
    if (!await allowDistributedRequest(client, request, { scope: 'payment-webhook', identity: orderId, limit: 30, windowMs: 60_000 })) {
      response.setHeader('Retry-After', '60')
      return sendError(response, 429, 'RATE_LIMITED', '웹훅 요청이 너무 많습니다.')
    }
