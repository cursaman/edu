...
-- 가입 당시 확인한 법적 문서 버전과 동의 시각을 변경 불가능한 이력으로 보관합니다.
create table if not exists public.user_consents (
  id bigint generated by default as identity primary key,
  user_id uuid not null references auth.users (id) on delete cascade,
  document_type text not null check (document_type in ('terms', 'privacy', 'age_confirmation')),
  document_version text not null check (char_length(document_version) between 1 and 80),
  agreed_at timestamptz not null default now(),
  source text not null default 'signup' check (source in ('signup', 'policy_update')),
...
begin
  ...
  if coalesce((new.raw_user_meta_data ->> 'edu_terms_agreed')::boolean, false) then
    insert into public.user_consents (user_id, document_type, document_version)
    values (new.id, 'terms', left(new.raw_user_meta_data ->> 'edu_terms_version', 80))
    on conflict do nothing;
  end if;
  if coalesce((new.raw_user_meta_data ->> 'edu_privacy_agreed')::boolean, false) then
    insert into public.user_consents (user_id, document_type, document_version)
    values (new.id, 'privacy', left(new.raw_user_meta_data ->> 'edu_privacy_version', 80))
    on conflict do nothing;
  end if;
  if coalesce((new.raw_user_meta_data ->> 'edu_age_confirmed')::boolean, false) then
    insert into public.user_consents (user_id, document_type, document_version)
    values (new.id, 'age_confirmation', left(new.raw_user_meta_data ->> 'edu_terms_version', 80))
    on conflict do nothing;
  end if;
  return new;
end;
...
-- INSERT는 Auth 가입 트리거만 수행하며 일반 사용자의 수정·삭제 정책은 만들지 않습니다.
