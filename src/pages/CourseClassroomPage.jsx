...
export default function CourseClassroomPage({ programId, sessionId }) {
  ...
  const guestLocked = Boolean(active && isSupabaseConfigured && !session?.user && active.order > 3)
  useEffect(() => {
    if (!session?.user?.id || !active) return undefined
    let activeRequest = true
    void Promise.all([loadUserLearning(session.user.id), recordLastSession(session.user.id, programId, active.id)])
      .then(([learning]) => { if (activeRequest) setCompleted(learning.completedByProgram[programId] || []) })
  ...
  return <section className="course-classroom page-shell">
    ...
    <div className="classroom-layout">
      ...
      {guestLocked ? <article className="classroom-content classroom-login-gate"><span aria-hidden="true">🔒</span><p className="section-eyebrow">FREE EXPERIENCE COMPLETE</p><h2>4회차부터는 로그인 후 이어서 학습할 수 있습니다</h2><p>회원가입하면 이 브라우저에서 완료한 1~3회차 진도를 계정으로 옮기고 다른 기기에서도 계속 학습할 수 있습니다.</p><div><a className="button button-primary" href={`#/signup?next=/classroom/${programId}/${active.id}`}>회원가입하고 이어서 학습하기 →</a><a className="button button-secondary" href={`#/login?next=/classroom/${programId}/${active.id}`}>이미 계정이 있어요</a></div></article> : <article className="classroom-content">
