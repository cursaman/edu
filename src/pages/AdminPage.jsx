...
export default function AdminPage({ session = null, onLogout }) {
  ...
  return (
    <section className="content-page page-shell admin-page" aria-labelledby="admin-title">
      ...
      <nav className="admin-navigation" aria-label="관리자 메뉴">{[['overview','운영 현황'],['quality','수업 품질 검증'],['programs','교육 프로그램 관리'],['lessons','교육자료 관리'],['notices','공지사항 관리'],['applications','체험 신청 확인']].map(([key,label]) => <button key={key} className={`filter-chip${section === key ? ' filter-chip-active' : ''}`} onClick={() => changeSection(key)} type="button">{label}</button>)}</nav>
