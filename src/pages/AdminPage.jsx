...
const emptyProgram = { title: '', categoryId: '', learningTrack: '입문', level: '입문', duration: '4주 · 토요일', status: '모집 예정', description: '', introduction: '', audience: '', goals: '', curriculum: '', preparations: '', relatedLessonIds: [], image: '', imageAlt: '', regularPrice: 0, salePrice: 0, isFree: true, saleStatus: 'draft' }
...
function ProgramForm({ editing, lessons, onCancel, onSave }) {
  ...
  return (
    <form className="admin-form" onSubmit={submit}>
      ...
      <label>학습 단계 <select onChange={(event) => updateField('learningTrack', event.target.value)} value={form.learningTrack || '입문'}><option>입문</option><option>실무</option><option>시니어</option></select></label>
      <label>난이도 <select onChange={(event) => updateField('level', event.target.value)} value={form.level}><option>입문</option><option>기초</option><option>중급</option><option>고급</option><option>실전</option><option>프로젝트</option></select></label>
