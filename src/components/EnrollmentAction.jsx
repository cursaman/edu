...
export default function EnrollmentAction({ course, programId, session }) {
  ...
  if (!session?.user) return <div className="enrollment-action"><strong>전체 과정 수강 신청</strong><p>첫 3회차는 로그인 없이 무료입니다. 4회차부터는 로그인 후 수강 승인이 필요합니다.</p><a className="button button-primary" href={`#/login?next=/programs/${programId}`}>로그인하고 신청하기</a></div>
  if (loading) return <div className="enrollment-action"><p role="status">수강 상태를 확인하고 있습니다...</p></div>
  if (!enrollment) return <div className="enrollment-action"><strong>전체 {course.sessions.length}회차 수강 신청</strong><p>이메일 계정만 사용하며 추가 개인정보는 받지 않습니다.</p><button className="button button-primary" onClick={apply} type="button">수강 신청하기</button>{message && <small role="status">{message}</small>}</div>

  const canLearn = allowedCourseStatuses.includes(enrollment.status)
  return <div className="enrollment-action"><span className={`enrollment-status enrollment-status-${enrollment.status}`}>{enrollmentStatuses[enrollment.status] || enrollment.status}</span><strong>{canLearn ? '전체 회차를 학습할 수 있습니다.' : '수강 신청 처리 상태'}</strong>{enrollment.status === 'pending' && <button className="button button-secondary" onClick={cancel} type="button">대기 신청 취소</button>}{canLearn && <a className="button button-primary" href={`#/classroom/${programId}/${course.sessions[0].id}`}>강의실 입장하기</a>}{message && <small role="status">{message}</small>}</div>
}
